...
//升级撤销权限
    ...
    function granAdmin(address _setAddress) external onlyLoging onlySuperAdmin{
        require(isRegister[_setAddress],"this user haven't registed,please go to register");
    ...
    function revokeAdmin(address _setAddress) external onlyLoging onlySuperAdmin{
        require(isAdmin[_setAddress],"this address is not admin");
        for(uint i=0;i<adminArray.length;i++){
            if(adminArray[i] == _setAddress){
...
//多签名交易
    ...
    function excute(uint _txId) 
        ...
        notExecuted(_txId)
    {
        require(_getApprovalAdmin(_txId) >= adminArray.length/2,"approver not enought");
    ...
    function adminNumber() external view returns(uint number){
        number = adminArray.length;
    }
